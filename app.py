# -*- coding: utf-8 -*-
# ============================================================
# MLB NON-K PROP ENGINE — ONE FILE — v12.0
...
APP_VERSION = "v12.1 NON-K HERO PROP ENGINE — PA + BULLPEN + HRRBI"
...
# =========================
st.set_page_config(
    page_title="MLB Non-K Prop Engine",
    layout="wide",
...
def render_kpis(picks, bankroll):
    """Non-K dashboard KPIs. Does not surface K picks or K betting signals."""
    pitcher_count = len(picks or [])
    confirmed = len([p for p in (picks or []) if p.get("lineup_locked")])
    fallback = max(0, pitcher_count - confirmed)
    ud_status = len([p for p in (picks or []) if str(p.get("underdog_status", "")).upper() == "FOUND"])
    st.markdown(f"""
    <div class="kpi-strip">
      <div class="kpi-box"><div class="kpi-label">Pitcher Pool</div><div class="kpi-value">{pitcher_count}</div><div class="kpi-sub">Used to build non-K props</div></div>
      <div class="kpi-box"><div class="kpi-label">Confirmed Lineups</div><div class="kpi-value green">{confirmed}</div><div class="kpi-sub">Fallback: {fallback}</div></div>
      <div class="kpi-box"><div class="kpi-label">Underdog Feed</div><div class="kpi-value green">{ud_status}</div><div class="kpi-sub">Matched source rows</div></div>
      <div class="kpi-box"><div class="kpi-label">Prop Families</div><div class="kpi-value">6</div><div class="kpi-sub">Outs / H / BB / ER / Hits / H+R+RBI</div></div>
      <div class="kpi-box"><div class="kpi-label">K Model</div><div class="kpi-value orange">OFF</div><div class="kpi-sub">Not shown / not graded here</div></div>
...
<div class="hero-panel">
  <div class="big-title">⚾ MLB NON-K PROP ENGINE v12.1</div>
  <div class="sub-title">Pitching Outs + Pitcher Props + Batter Props — PA, bullpen, handedness, HRRBI correlation</div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    hide_no_line = st.checkbox("Underlying pitcher K-line filter only (recommended OFF)", value=False)
    only_strong = st.checkbox("Underlying pitcher filter only (recommended OFF for non-K props)", value=False)
    st.divider()
...
with col_save:
    save_btn = st.button("💾 NON-K SNAPSHOT NOTE", use_container_width=True)
...
if save_btn:
    st.info("This separate non-K app does not write to the K model save/grade logs. Use the Snapshot tab after refreshing to export non-K prop rows safely.")
...
# Non-K app: keep the full pitcher pool so outs/pitcher/batter props can build rows.
# Do not filter the underlying board by K line or K signal quality.
if False and hide_no_line:
    board = [p for p in board if p.get("line") is not None]
if False and only_strong:
...
def prop_confidence_from_edge(edge, volatility="MED", data_quality=70, line_is_real=True):
    if not line_is_real:
        return None
    e = abs(safe_float(edge, 0) or 0)
    vol = str(volatility or "MED").upper()
    q = safe_float(data_quality, 70) or 70
    base = 46 + min(e * 12, 28) + clamp((q - 65) * 0.22, -8, 10)
    if vol == "LOW":
        base += 7
    elif vol == "MED":
        base += 1
...
def prop_tier_from_confidence(conf, pick, volatility="MED"):
    ...
    vol = str(volatility or "MED").upper()
    if vol == "EXTREME":
        return "TRAP" if conf < 86 else "RISKY"
    if vol == "HIGH" and conf < 80:
        return "RISKY"
    if conf >= 86 and vol in ["LOW", "MED"]:
        return "ELITE"
    if conf >= 76:
        return "STRONG"
    if conf >= 64:
...
# =========================
# v12.1 NON-K HERO MODEL HELPERS
# =========================
def non_k_storage_path(name):
    return os.path.join(STORAGE_DIR, name)

NON_K_CALIBRATION_FILE = non_k_storage_path("non_k_calibration_engine.json")
NON_K_RESULT_LOG = non_k_storage_path("non_k_result_log.json")

LEAGUE_AVG_HIT_RATE_PER_PA = 0.235
LEAGUE_AVG_BB_RATE_PER_PA = 0.082
LEAGUE_AVG_RUN_PER_PA = 0.115
LEAGUE_AVG_RBI_PER_PA = 0.105
LINEUP_SLOT_PA_BASE = {1: 4.75, 2: 4.62, 3: 4.50, 4: 4.38, 5: 4.24, 6: 4.08, 7: 3.92, 8: 3.76, 9: 3.60}

def non_k_calibration_profile(prop_type=None):
    rows = load_json(NON_K_RESULT_LOG, [])
    usable = []
    for r in rows:
        if prop_type and str(r.get("prop_type") or r.get("Prop") or "").lower() != str(prop_type).lower():
            continue
        proj = safe_float(r.get("projection", r.get("Projection")))
        actual = safe_float(r.get("actual", r.get("Actual")))
        if proj is None or actual is None:
            continue
        usable.append((proj, actual))
    if not usable:
        return {"samples": 0, "bias": 0.0, "mae": None, "quality": 52, "note": "Non-K calibration warming up"}
    errors = [a - p for p, a in usable]
    mae = float(np.mean([abs(e) for e in errors]))
    bias = float(np.mean(errors))
    quality = int(clamp(56 + min(len(usable), 120) * 0.22 - min(mae, 4.0) * 5.5 - abs(bias) * 3.5, 1, 100))
    return {"samples": len(usable), "bias": round(bias, 3), "mae": round(mae, 3), "quality": quality, "note": "Separate non-K calibration"}

def apply_non_k_calibration(proj, prop_type):
    profile = non_k_calibration_profile(prop_type)
    p = safe_float(proj)
    if p is None:
        return proj, profile, "No non-K projection to calibrate"
    if profile.get("samples", 0) < 20:
        return p, profile, f"Non-K calibration warming up ({profile.get('samples',0)}/20)"
    shift = clamp((safe_float(profile.get("bias"), 0) or 0) * 0.18, -0.20, 0.20)
    return round(max(0, p + shift), 2), profile, f"Non-K calibration shift {shift:+.2f} ({prop_type})"

def lineup_slot_pa(slot, lineup_confirmed=True, team_total=None):
    slot = int(slot or 9)
    pa = LINEUP_SLOT_PA_BASE.get(slot, 3.55)
    if not lineup_confirmed:
        pa -= 0.22
    tt = safe_float(team_total)
    if tt is not None:
        if tt >= 5.2: pa += 0.10
        elif tt <= 3.6: pa -= 0.10
    return round(clamp(pa, 3.15, 5.05), 2)

def batter_rate_from_row(row, keys, default):
    for k in keys:
        val = safe_float((row or {}).get(k))
        if val is not None:
...
def batter_contact_index(row):
    k_rate = batter_rate_from_row(row, ["K%", "K Rate", "strikeout_rate", "k_rate"], 0.225)
    avg = batter_rate_from_row(row, ["AVG", "avg", "batting_average"], 0.245)
    obp = batter_rate_from_row(row, ["OBP", "obp"], 0.315)
    contact = 1.0 + clamp((0.225 - k_rate) * 1.15, -0.14, 0.16) + clamp((avg - 0.245) * 0.85, -0.10, 0.13) + clamp((obp - 0.315) * 0.45, -0.06, 0.08)
    return clamp(contact, 0.72, 1.35), k_rate, avg, obp

def handedness_matchup_factor(batter_row, pitcher_hand=None):
    ph = str(pitcher_hand or "").upper()[:1]
    bat_side = str((batter_row or {}).get("Bats") or (batter_row or {}).get("bat_side") or (batter_row or {}).get("side") or "").upper()[:1]
    if not ph or not bat_side:
        return 1.00, "Handedness neutral"
    if bat_side == "S":
        return 1.025, "Switch hitter stability"
    if (bat_side == "L" and ph == "R") or (bat_side == "R" and ph == "L"):
        return 1.035, "Platoon advantage"
    return 0.975, "Same-side matchup penalty"

def bullpen_matchup_factor(picklike):
    status = str((picklike or {}).get("bullpen_status") or "").upper()
    risk = str((picklike or {}).get("run_damage_risk_level") or (picklike or {}).get("game_script_risk") or "").upper()
    factor = 1.00
    notes = []
    if "TIRED" in status or "BAD" in status or "THIN" in status:
        factor += 0.045; notes.append("Bullpen weakness boost")
    if risk in ["HIGH", "EXTREME"]:
        factor += 0.045; notes.append("Run/traffic environment boost")
    elif risk == "MILD":
        factor += 0.020; notes.append("Mild run environment boost")
    if not notes:
        notes.append("Bullpen neutral")
    return clamp(factor, 0.92, 1.12), ", ".join(notes)

def hitter_volatility(row, lineup_slot, lineup_confirmed=True, market="Batter Hits", projection=None):
    contact, k_rate, avg, obp = batter_contact_index(row)
    slot = int(lineup_slot or 9)
    score = 0; notes = []
    if not lineup_confirmed: score += 3; notes.append("lineup not confirmed")
    if slot >= 7: score += 2; notes.append("bottom-third lineup slot")
    elif slot <= 3: score -= 1; notes.append("premium lineup slot")
    if k_rate >= 0.285: score += 2; notes.append("high K hitter")
    elif k_rate <= 0.175: score -= 1; notes.append("contact hitter")
    if str(market).upper().replace(" ", "") in ["H+R+RBI", "HRRBI"]: score += 1; notes.append("combo stat variance")
    proj = safe_float(projection)
    if proj is not None and proj <= 0.75: score += 1; notes.append("thin projection")
    if score >= 5: return "EXTREME", "; ".join(notes) or "very high hitter variance"
    if score >= 3: return "HIGH", "; ".join(notes) or "high hitter variance"
    if score <= 0: return "LOW", "; ".join(notes) or "stable hitter profile"
    return "MED", "; ".join(notes) or "normal hitter variance"

def hrrbi_correlation_multiplier(slot, team_run_factor=1.0, contact_factor=1.0, bullpen_factor=1.0):
    slot = int(slot or 9)
    if slot in [1, 2]: slot_corr = 1.10
    elif slot in [3, 4]: slot_corr = 1.15
    elif slot == 5: slot_corr = 1.08
    elif slot in [6, 7]: slot_corr = 1.00
    else: slot_corr = 0.94
    raw = slot_corr * clamp(team_run_factor, 0.90, 1.14) * clamp(0.96 + (contact_factor - 1.0) * 0.45, 0.92, 1.08) * clamp(0.97 + (bullpen_factor - 1.0) * 0.55, 0.94, 1.08)
    return clamp(raw, 0.86, 1.22)

def pitcher_non_k_quality(p, market):
    score = 70
    if p.get("lineup_locked"): score += 8
    if safe_float(p.get("expected_bf"), 0) >= 23: score += 5
    if str(p.get("leash_risk") or "").upper() in ["SHORT_RECENT_STARTS", "HIGH_PITCH_COUNT", "HIGH_RECENT_WORKLOAD"]: score -= 10
    if str(market).lower() in ["earned runs", "hits allowed"] and str(p.get("run_damage_risk_level") or "").upper() in ["HIGH", "EXTREME"]: score += 3
    return int(clamp(score, 1, 99))

def batter_non_k_quality(row, slot, lineup_confirmed, market):
    score = 62
    if lineup_confirmed: score += 12
    if int(slot or 9) <= 4: score += 8
    elif int(slot or 9) >= 8: score -= 7
    contact, k_rate, avg, obp = batter_contact_index(row)
    score += clamp((contact - 1.0) * 32, -10, 12)
    if str(market).upper().replace(" ", "") in ["H+R+RBI", "HRRBI"] and int(slot or 9) in [1,2,3,4,5]: score += 5
    return int(clamp(score, 1, 99))

# =========================
# v11.21 SEPARATE UNDERDOG PROP LINE ENGINE
# =========================
# This parser is intentionally separate from the K engine. It can attach real
# Underdog lines to pitcher/batter prop tabs, but it does not change strikeout
# projections, K calibration, K grading, or K pick decisions.
def underdog_market_aliases(market_key):
    key = str(market_key or "").lower().strip()
    aliases = {
        "pitching outs": ["pitching outs", "outs recorded", "pitcher outs", "outs"],
        "hits allowed": ["hits allowed", "pitcher hits allowed"],
        "walks allowed": ["walks allowed", "pitcher walks", "bases on balls allowed", "bb allowed"],
        "earned runs": ["earned runs", "earned runs allowed", "pitcher earned runs"],
        "batter hits": ["batter hits", "hits"],
        "h+r+rbi": ["hits + runs + rbis", "hits+runs+rbi", "hits runs rbis", "h+r+rbi", "hrrbi"],
    }
    return aliases.get(key, [key])

def underdog_market_blob_matches(blob, market_key):
    blob = str(blob or "").lower().replace("_", " ").replace("-", " ")
    key = str(market_key or "").lower().strip()
    aliases = underdog_market_aliases(key)

    # Hard exclusions so markets do not bleed into each other.
    if key == "batter hits":
        bad = ["hits + runs", "hits+runs", "h+r+rbi", "runs + rbis", "total bases", "pitcher hits allowed", "hits allowed"]
        if any(x in blob for x in bad):
            return False
        return any(a in blob for a in aliases) and not any(x in blob for x in ["pitcher", "allowed"])

    if key == "h+r+rbi":
        return any(a in blob for a in aliases)

    if key == "hits allowed":
        return ("hits allowed" in blob or "pitcher hits allowed" in blob) and "batter" not in blob

    if key == "walks allowed":
        return any(a in blob for a in aliases) and "batter" not in blob

    if key == "earned runs":
        return any(a in blob for a in aliases) and "batter" not in blob

    if key == "pitching outs":
        # Prefer full phrases so generic "outs" from unrelated baseball text does not match.
        return any(a in blob for a in ["pitching outs", "outs recorded", "pitcher outs"])

    return any(a in blob for a in aliases)

def underdog_generic_line_from_objects(*objs):
    ...
    for obj in objs:
        ...
        if isinstance(obj.get("attributes"), dict):
            a.update(obj.get("attributes") or {})
        for k, v in obj.items():
            if k not in ["attributes", "relationships", "included", "data"] and k not in a:
                a[k] = v
        for k in safe_keys:
            val = safe_float(a.get(k))
            if val is not None and 0 <= val <= 40:
                # Most props are half-point lines; allow integers for outs and earned/run markets.
                return float(val), f"{k} from Underdog"
    text = " | ".join(json.dumps(o, default=str)[:500] for o in objs if isinstance(o, dict))
    vals = extract_half_lines_from_text(text)
    if vals:
        return float(vals[0]), "half-line from Underdog text"
    return None, "no usable Underdog line"

@st.cache_data(ttl=300, show_spinner=False)
def get_underdog_prop_line(player_name, market_key):
    """Return a real Underdog line for a non-K prop without touching K logic.

    This is separate by design:
    - Pitcher props use pitcher names + pitcher prop market labels.
    - Batter props use batter names + batter prop market labels.
    - No fake line is created. If no real match is found, status = NO MATCH.
    """
    player_name = str(player_name or "").strip()
    market_key = str(market_key or "").strip()
    if not player_name or not market_key:
        return source_result("Underdog", "NO MATCH", rows=[], message="Missing player or market")
    ...
    def attrs2(obj):
        ...
        out = {}
        if isinstance(obj.get("attributes"), dict):
            out.update(obj.get("attributes") or {})
        for k, v in obj.items():
            if k not in ["attributes", "relationships", "included", "data"] and k not in out:
                out[k] = v
        return out
    ...
    def obj_type2(obj):
        return str(obj.get("type") or "").lower().replace("-", "_") if isinstance(obj, dict) else ""

    def obj_id2(obj):
        if not isinstance(obj, dict):
            return None
        val = obj.get("id") or attrs2(obj).get("id")
        return str(val) if val not in [None, ""] else None

    def rel_id2(obj, rel_names):
        if not isinstance(obj, dict):
            return None
        rels = obj.get("relationships") or {}
        for name in rel_names:
            for cname in [name, name.replace("_", "-"), name.replace("_", "")]:
                node = rels.get(cname)
                if node is None:
                    continue
                data = node.get("data") if isinstance(node, dict) else node
    ...
    def player_from2(player_obj, appearance_obj=None, line_obj=None, ou_obj=None):
        candidates = []
    ...
    def status_ok2(*objs):
        ...
        return not any(x in blob for x in ["suspended", "removed", "hidden", "inactive", "closed", "disabled"])
    ...
    for url in UNDERDOG_URLS:
        ...
        if not data:
            last_msg = f"No JSON from {url}"
            continue
        objects = collect_objects2(data)
        by_type_id = {}
        for obj in objects:
            oid = obj_id2(obj)
            if oid:
                by_type_id[(obj_type2(obj), oid)] = obj

        # Relationship mode: line -> over_under -> appearance -> player.
        for line_obj in objects:
            if obj_type2(line_obj) not in {"over_under_line", "over_under_lines"}:
                continue
            ou_id = rel_id2(line_obj, ["over_under", "over_unders"])
            ou_obj = None
            if ou_id:
                for t in ["over_under", "over_unders"]:
                    ou_obj = by_type_id.get((t, ou_id)) or ou_obj
            app_id = rel_id2(ou_obj or line_obj, ["appearance", "appearances"])
            app_obj = None
            if app_id:
                for t in ["appearance", "appearances"]:
                    app_obj = by_type_id.get((t, app_id)) or app_obj
            player_id = rel_id2(app_obj or ou_obj or line_obj, ["player", "players"])
            player_obj = None
            if player_id:
                for t in ["player", "players"]:
                    player_obj = by_type_id.get((t, player_id)) or player_obj

            evidence = text_from2(line_obj, ou_obj, app_obj, player_obj)
            if is_bad_sport_text(evidence):
                continue
            if not underdog_market_blob_matches(evidence, market_key):
                continue
            matched_name = player_from2(player_obj, app_obj, line_obj, ou_obj)
            score = max(name_score(player_name, matched_name), name_score(player_name, evidence))
            if score < 0.84:
                continue
            if not status_ok2(line_obj, ou_obj, app_obj, player_obj):
                continue
            line, line_note = underdog_generic_line_from_objects(line_obj, ou_obj, app_obj)
            if line is None:
                continue
            accepted.append({
                "Source": "Underdog", "Provider": "Underdog", "Player": player_name,
                "Matched Name": matched_name or evidence[:120], "Match Score": round(float(score), 3),
                "Market": market_key, "Side": "OVER/UNDER", "Line": float(line), "Price": None,
                "Line Evidence": line_note, "Parser Mode": "relationship", "Underdog Path": f"line:{obj_id2(line_obj)} -> ou:{ou_id} -> app:{app_id} -> player:{player_id}",
            })

        # Recursive fallback: only accept if the same object/text clearly carries both player and market.
        for obj in objects:
            blob = json.dumps(obj, default=str)[:2000]
            if is_bad_sport_text(blob):
                continue
            if not underdog_market_blob_matches(blob, market_key):
                continue
            score = name_score(player_name, blob)
            if score < 0.88:
                continue
            if not status_ok2(obj):
                continue
            line, line_note = underdog_generic_line_from_objects(obj)
            if line is None:
                continue
            accepted.append({
                "Source": "Underdog", "Provider": "Underdog", "Player": player_name,
                "Matched Name": player_name, "Match Score": round(float(score), 3),
                "Market": market_key, "Side": "OVER/UNDER", "Line": float(line), "Price": None,
                "Line Evidence": line_note, "Parser Mode": "recursive", "Underdog Path": f"obj:{obj_id2(obj) or 'unknown'}",
            })
    ...
    if not accepted:
        return source_result("Underdog", "NO MATCH", rows=[], message=last_msg or f"No real Underdog {market_key} line matched")

    dedup = {}
    for r in accepted:
        key = (round(safe_float(r.get("Line"), -1), 2), r.get("Matched Name"), r.get("Parser Mode"))
        if key not in dedup or safe_float(r.get("Match Score"), 0) > safe_float(dedup[key].get("Match Score"), 0):
            dedup[key] = r
    rows = list(dedup.values())
    # Prefer relationship rows, then highest name score, then normal half-lines.
    def rank(r):
        rel = 1 if r.get("Parser Mode") == "relationship" else 0
        half = 1 if is_half_point_line(r.get("Line")) else 0
        return (rel, safe_float(r.get("Match Score"), 0) or 0, half)
    best = sorted(rows, key=rank, reverse=True)[0]
    return source_result(
        "Underdog", "FOUND", line=float(best["Line"]), rows=rows,
        message=f"Live Underdog {market_key} line matched: {float(best['Line']):.1f} via {best.get('Matched Name')} ({best.get('Parser Mode')})"
    )

def prop_line_lookup(p, market_key):
    """Separate real-line lookup for non-K pitcher props.

    First checks explicitly attached real fields, then separately searches Underdog
    for the requested market. This does not touch the K line parser or K decision engine.
    """
    field_map = {
    ...
    pitcher = p.get("pitcher") or p.get("Player") or p.get("Name")
    if pitcher:
        ud = get_underdog_prop_line(str(pitcher), str(market_key))
        line = safe_float(ud.get("line")) if isinstance(ud, dict) else None
        if ud.get("status") == "FOUND" and line is not None:
            return line, f"Underdog Live Exact — {market_key}"
    return None, "NO LINE"
...
def pitcher_prop_base_rows(board):
    ...
    for p in board or []:
        ...
        lineup_confirmed = bool(p.get("lineup_locked"))
        bullpen_factor, bullpen_note = bullpen_matchup_factor(p)
        base_volatility = "HIGH" if leash in ["SHORT_RECENT_STARTS", "HIGH_PITCH_COUNT", "HIGH_RECENT_WORKLOAD"] or not lineup_confirmed else "MED"
        if ppb <= 3.75 and lineup_confirmed:
            base_volatility = "LOW"
        outs_proj = round(clamp((bf * 0.72) * (0.98 if base_volatility == "HIGH" else 1.02 if base_volatility == "LOW" else 1.0), 9, 24), 1)
        traffic_boost = 0.45 if str(p.get("run_damage_risk_level") or "").upper() in ["HIGH", "EXTREME"] else 0.20 if str(p.get("run_damage_risk_level") or "").upper() == "MILD" else 0.0
        hits_proj = round(clamp((bf * LEAGUE_AVG_HIT_RATE_PER_PA * bullpen_factor) + traffic_boost, 2.3, 10.2), 1)
        command_boost = 0.28 if ppb >= 4.10 else 0.12 if ppb >= 3.95 else 0.0
        walks_proj = round(clamp((bf * LEAGUE_AVG_BB_RATE_PER_PA) + command_boost + (0.12 if base_volatility == "HIGH" else 0), 0.4, 5.4), 1)
        er_proj = round(clamp(((hits_proj * 0.34) + (walks_proj * 0.20)) * clamp(bullpen_factor, 0.95, 1.10), 0.3, 6.4), 1)
        markets = [("Pitching Outs", outs_proj, "Starter volume + leash estimate"),("Hits Allowed", hits_proj, "Contact + traffic + bullpen matchup estimate"),("Walks Allowed", walks_proj, "Command + pitch efficiency estimate"),("Earned Runs", er_proj, "Traffic + run prevention + bullpen context")]
        for market, raw_proj, note in markets:
            proj, cal_profile, cal_note = apply_non_k_calibration(raw_proj, market)
            line, source = prop_line_lookup(p, market)
            pick = prop_pick_from_projection(proj, line)
            edge = prop_edge(proj, line)
            q = pitcher_non_k_quality(p, market)
            conf = prop_confidence_from_edge(edge, base_volatility, q, line is not None)
            tier = prop_tier_from_confidence(conf, pick, base_volatility)
            rows.append({"Tier": tier, "Pitcher": pitcher, "Matchup": matchup, "Prop": market, "Projection": proj, "Line": line if line is not None else "NO LINE", "Pick": pick, "Edge": edge, "Confidence": conf if line is not None else None, "Volatility": base_volatility, "Data Quality": q, "Line Source": source, "Lineup": lineup_status, "Bullpen": bullpen_note, "Calibration": cal_note, "Note": note if line is not None else prop_no_line_note()})
    return pd.DataFrame(rows)
...
def batter_prop_rows(board):
    ...
    for p in board or []:
        ...
        lineup_status = p.get("lineup_status") or ("CONFIRMED" if p.get("lineup_locked") else "FALLBACK")
        lineup_confirmed = bool(p.get("lineup_locked"))
        pitcher_hand = p.get("hand") or p.get("pitcher_hand")
        bullpen_factor, bullpen_note = bullpen_matchup_factor(p)
        team_total = safe_float(p.get("team_total") or p.get("implied_total") or p.get("opp_implied_total"))
        team_run_factor = 1.0
        if team_total is not None:
            team_run_factor = 1.08 if team_total >= 5.0 else 0.94 if team_total <= 3.6 else 1.0
        elif str(p.get("run_damage_risk_level") or "").upper() in ["HIGH", "EXTREME"]:
            team_run_factor = 1.05
        for idx, r in enumerate((p.get("lineup_rows") or [])[:9], start=1):
            ...
            if key in seen:
                continue
            seen.add(key)
            pa = lineup_slot_pa(idx, lineup_confirmed, team_total)
            contact_factor, k_rate, avg, obp = batter_contact_index(r)
            hand_factor, hand_note = handedness_matchup_factor(r, pitcher_hand)
            hit_rate = clamp(LEAGUE_AVG_HIT_RATE_PER_PA * contact_factor * hand_factor, 0.145, 0.365)
            hits_raw = round(clamp(pa * hit_rate, 0.32, 1.75), 2)
            corr = hrrbi_correlation_multiplier(idx, team_run_factor, contact_factor, bullpen_factor)
            run_component = pa * LEAGUE_AVG_RUN_PER_PA * team_run_factor * (1.10 if idx <= 2 else 1.0)
            rbi_component = pa * LEAGUE_AVG_RBI_PER_PA * team_run_factor * (1.18 if idx in [3,4,5] else 0.92 if idx <= 2 else 1.0)
            hrrbi_raw = round(clamp((hits_raw + run_component + rbi_component) * corr * clamp(bullpen_factor, 0.95, 1.10), 0.55, 4.25), 2)
            for market, raw_proj, note in [("Batter Hits", hits_raw, "PA simulation + contact + handedness estimate"),("H+R+RBI", hrrbi_raw, "PA volume + lineup slot + run/RBI correlation logic")]:
                proj, cal_profile, cal_note = apply_non_k_calibration(raw_proj, market)
                ud = get_underdog_prop_line(str(name), str(market))
                line = safe_float(ud.get("line")) if isinstance(ud, dict) and ud.get("status") == "FOUND" else None
                source = f"Underdog Live Exact — {market}" if line is not None else "NO LINE"
                pick = prop_pick_from_projection(proj, line)
                edge = prop_edge(proj, line)
                vol, vol_note = hitter_volatility(r, idx, lineup_confirmed, market, proj)
                q = batter_non_k_quality(r, idx, lineup_confirmed, market)
                conf = prop_confidence_from_edge(edge, vol, q, line is not None)
                tier = prop_tier_from_confidence(conf, pick, vol) if line is not None else "TRACKING"
                rows.append({"Tier": tier, "Batter": name, "Matchup": matchup, "Lineup Slot": idx, "Prop": market, "Projection": proj, "Line": line if line is not None else "NO LINE", "Pick": pick, "Edge": edge, "Confidence": conf, "Volatility": vol, "Data Quality": q, "PA Proj": pa, "Handedness": hand_note, "Bullpen": bullpen_note, "HRRBI Corr": round(corr, 3) if market == "H+R+RBI" else "—", "Line Source": source, "Lineup": lineup_status, "Calibration": cal_note, "Note": note + (f" | {vol_note}" if vol_note else "") if line is not None else prop_no_line_note()})
    return pd.DataFrame(rows)
...
def render_prop_dataframe(title, df, subtitle=""):
    ...
    if df is None or df.empty:
        ...
        return
    cols = [c for c in ["Tier", "Pitcher", "Batter", "Matchup", "Lineup Slot", "Prop", "Projection", "Line", "Pick", "Edge", "Confidence", "Volatility", "Data Quality", "PA Proj", "Handedness", "HRRBI Corr", "Bullpen", "Calibration", "Line Source", "Lineup", "Note"] if c in df.columns]
    st.dataframe(df[cols], use_container_width=True, hide_index=True)

def build_non_k_combined_board(board):
    """Combine only non-K prop rows. Strikeouts are intentionally excluded."""
    pdf = pitcher_prop_base_rows(board)
    bdf = batter_prop_rows(board)
    combined = pd.concat([x for x in [pdf, bdf] if x is not None and not x.empty], ignore_index=True, sort=False)
    if combined.empty:
        return combined
    # Make sure no K/strikeout rows sneak into this separate app.
    if "Prop" in combined.columns:
        combined = combined[~combined["Prop"].astype(str).str.lower().str.contains("strikeout|ks| k ", regex=True, na=False)].copy()
    # Sort strongest/most actionable first.
    tier_order = {"ELITE": 0, "STRONG": 1, "LEAN": 2, "RISKY": 3, "TRACKING": 4, "PASS": 5, "NO LINE": 6}
    if "Tier" in combined.columns:
        combined["_tier_rank"] = combined["Tier"].astype(str).str.upper().map(tier_order).fillna(9)
    else:
        combined["_tier_rank"] = 9
    if "Confidence" in combined.columns:
        combined["_conf_sort"] = pd.to_numeric(combined["Confidence"], errors="coerce").fillna(0)
    else:
        combined["_conf_sort"] = 0
    if "Edge" in combined.columns:
        combined["_edge_sort"] = pd.to_numeric(combined["Edge"], errors="coerce").abs().fillna(0)
    else:
        combined["_edge_sort"] = 0
    combined = combined.sort_values(["_tier_rank", "_conf_sort", "_edge_sort"], ascending=[True, False, False])
    return combined.drop(columns=[c for c in ["_tier_rank", "_conf_sort", "_edge_sort"] if c in combined.columns])


def render_main_multi_prop_board(board):
    st.markdown('<div class="section-title-pro">Main Board — Non-K Props Only</div>', unsafe_allow_html=True)
    st.caption("Strikeouts/K picks are intentionally removed. This app is separate from the K model and does not grade or save K picks.")
    if not board:
        st.info("Click Refresh Live Board first.")
        return
    combined = build_non_k_combined_board(board)
    if combined.empty:
        st.info("No non-K prop rows yet. Refresh the live board and check Underdog line availability.")
        return
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Non-K Rows", len(combined))
    c2.metric("Pitcher Prop Rows", int(combined["Pitcher"].notna().sum()) if "Pitcher" in combined else 0)
    c3.metric("Batter Prop Rows", int(combined["Batter"].notna().sum()) if "Batter" in combined else 0)
    if "Line Source" in combined.columns:
        real_lines = int((combined["Line Source"].astype(str) != "NO LINE").sum())
    else:
        real_lines = 0
    c4.metric("Real Underdog Lines", real_lines)
    cols = [c for c in ["Tier", "Pitcher", "Batter", "Matchup", "Lineup Slot", "Prop", "Projection", "Line", "Pick", "Edge", "Confidence", "Volatility", "Data Quality", "PA Proj", "Handedness", "HRRBI Corr", "Bullpen", "Calibration", "Line Source", "Lineup", "Note"] if c in combined.columns]
...
tab_main_multi, tab_outs, tab_pitcher_props, tab_batter_props, tab_all, tab_snapshot, tab_settings = st.tabs([
    "MAIN BOARD",
    "PITCHING OUTS",
    "PITCHER PROPS",
    "BATTER PROPS",
...
with tab_all:
    st.markdown('<div class="section-title-pro">All Underlying Game Data</div>', unsafe_allow_html=True)
    st.caption("This is the pitcher/game pool used to build non-K props. K columns may exist internally but K picks are not displayed or graded in this separate app.")
    if board:
        show = pd.DataFrame([{k: v for k, v in p.items() if k not in ["prop_rows", "lineup_rows", "pitch_type_rows"]} for p in board])
        safe_cols = [
            "date", "pitcher", "matchup", "hand", "team", "opponent", "expected_bf", "ppb", "recent_ip",
            "lineup_status", "lineup_locked", "pitcher_confirmed", "bullpen_status", "bullpen_bf_factor",
            "weather_note", "umpire_note", "data_score", "underdog_status", "underdog_message"
        ]
        cols = [c for c in safe_cols if c in show.columns]
        st.dataframe(show[cols], use_container_width=True, hide_index=True)
    else:
        st.info("No data loaded yet.")

with tab_snapshot:
    st.markdown('<div class="section-title-pro">Snapshot / Export — Non-K Props Only</div>', unsafe_allow_html=True)
    combined = build_non_k_combined_board(board) if board else pd.DataFrame()
    if combined.empty:
        st.info("Refresh the board first. Then export or save a non-K snapshot here.")
    else:
        cols = [c for c in ["Tier", "Pitcher", "Batter", "Matchup", "Lineup Slot", "Prop", "Projection", "Line", "Pick", "Edge", "Confidence", "Volatility", "Data Quality", "PA Proj", "Handedness", "HRRBI Corr", "Bullpen", "Calibration", "Line Source", "Lineup", "Note"] if c in combined.columns]
        clean = combined[cols].copy()
        st.dataframe(clean, use_container_width=True, hide_index=True)
        csv = clean.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download Non-K Prop CSV", data=csv, file_name=f"non_k_props_{california_now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True)
        if st.button("💾 Save Non-K Snapshot JSON", use_container_width=True):
            path = os.path.join(STORAGE_DIR, "non_k_prop_snapshot_log.json")
            rows = load_json(path, [])
            stamp = now_iso()
            for r in clean.to_dict("records"):
                r["snapshot_time"] = stamp
                r["app_version"] = APP_VERSION
                rows.append(r)
            save_json(path, rows[-20000:])
            st.success(f"Saved {len(clean)} non-K rows to non_k_prop_snapshot_log.json. K model logs were not touched.")

with tab_settings:
    st.markdown('<div class="section-title-pro">Settings / Safety Notes</div>', unsafe_allow_html=True)
    st.success("This is a separate non-K prop app. Strikeout/K picks are hidden and K grading logs are not written from this screen.")
    st.write("Use the K-only Hero app for strikeouts. Use this app for Pitching Outs, Hits Allowed, Walks, Earned Runs, Batter Hits, and H+R+RBI.")
    st.write("Real lines come from the existing Underdog lookup helpers when available. No fake lines are created.")
